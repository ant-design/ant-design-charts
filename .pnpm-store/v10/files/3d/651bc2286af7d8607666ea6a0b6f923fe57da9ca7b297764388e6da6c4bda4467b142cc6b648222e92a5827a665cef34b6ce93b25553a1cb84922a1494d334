...
var Critters = _interopDefault(require('critters'));
 ...
 * the License.
 */
const $require = typeof require !== 'undefined' ? require : module$1.createRequire(eval('import.meta.url')); // Used to annotate this plugin's hooks in Tappable invocations
...
class CrittersWebpackPlugin extends Critters {
  constructor(options) {
  ...
  async getCssAsset(href, style) {
    ...
    const filename = path.resolve(outputPath, normalizedPath); // try to find a matching asset by filename in webpack's output (not yet written to disk)
  ...
  pruneSource(style, before, sheetInverse) {
    ...
    if (asset) {
      ...
      this.compilation.assets[style.$$assetName] = new sources.LineToLineMappedSource(sheetInverse, style.$$assetName, before);
    } else {
...
module.exports = CrittersWebpackPlugin;
