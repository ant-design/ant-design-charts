...
function interpolateLine(coordinates, m) {
    ...
    while(++i < n){
        ...
        dx = (p1[0] - p0[0]) / m;
        dy = (p1[1] - p0[1]) / m;
        for(var j = 0; j < m; ++j)resampled.push([
            p0[0] + j * dx,
            p0[1] + j * dy
        ]);
        p0 = p1;
...
function _default(project, lobes, inverse) {
        ...
        forward.invert = function(x, y) {
            ...
            for(var i = 0, n = bound.length; i < n; ++i){
                ...
                if (b[0][0] <= x && x < b[1][0] && b[0][1] <= y && y < b[1][1]) {
                    ...
                    p[0] += lobe[i][1][0];
                    return pointEqual(forward(p[0], p[1]), [
                        x,
                        y
                    ]) ? p : null;
                }
    ...
    var p = (0, _index1.geoProjection)(forward), stream_ = p.stream;
