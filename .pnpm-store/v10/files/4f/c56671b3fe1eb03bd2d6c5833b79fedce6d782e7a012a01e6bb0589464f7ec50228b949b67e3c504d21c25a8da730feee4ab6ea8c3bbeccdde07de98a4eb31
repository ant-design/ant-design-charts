...
class FileSystemCache {
    constructor(options = {}) {
        ...
        this.hash = (_a = options.hash) !== null && _a !== void 0 ? _a : 'sha1';
        this.ns = common_1.Util.hash(this.hash, options.ns);
        this.ttl = (_b = options.ttl) !== null && _b !== void 0 ? _b : 0;
        ...
        if (!common_1.Util.hashExists(this.hash)) {
    ...
    path(key) {
        ...
        let name = common_1.Util.hash(this.hash, key);
        if (this.ns)
            name = `${this.ns}-${name}`;
        if (this.extension)
...
FileSystemCache.hashAlgorithms = common_1.hashAlgorithms;
