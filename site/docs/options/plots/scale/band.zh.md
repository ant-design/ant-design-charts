...
band 比例尺是一种特殊的分类比例尺，它将离散的定义域（如类别、名称等）映射到连续的数值范围，并为每个类别分配相等宽度的区间（band）。与普通的 [ordinal](/options/plots/core/scale/ordinal) 比例尺不同，band 比例尺不仅关注点的位置，还关注每个类别占据的区间宽度。
...
在 Ant Design Charts 中，band 比例尺是柱状图（interval 标记）的默认 x 轴比例尺，它能够自动处理类别数据的映射和布局。
...
const Demo = () => {
  ...
  const config ={
      ...
      colorField: 'region',
      // 根据人口数据调整柱宽
      transform: [{ type: 'flexX', field: 'population' }],
      scale: {
        x: {
          type: 'band',
          padding: 0.2
...
const Demo = () => {
  ...
  const config ={
      ...
      xField: 'month',
      yField: 'sales',
      colorField: (d) => d.sales > 1500 ? 'high' : d.sales > 1300 ? 'medium' : 'low',
      scale: {
        x: {
          type: 'band',
          padding: 0.1
...
const Demo = () => {
  ...
  const config ={
      ...
      xField: 'name',
      yField: ['position', (d) => d.position + d.value],
      colorField: 'type',
      scale: {
        x: {
          type: 'band',
