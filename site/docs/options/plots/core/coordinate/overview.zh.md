...
在可视化的视觉通道(Channels)中，位置(Position)是最具有感知优势的视觉编码方式。它不仅具有最高的识别准确度，而且能够同时有效地表达定性数据（类别）和定量数据。为了系统地组织这种空间位置映射，我们引入了 **坐标系（Coordinate）** 的概念。在 Ant Design Charts 的实现中，坐标系统负责管理一系列的空间变换。具体来说，图形标记（Mark）的位置属性（x 和 y）首先会通过比例尺（Scale）映射到标准化的 `[0, 1]` 区间，随后坐标系统会将这些标准化的位置信息转换为最终的画布坐标，从而实现不同空间布局形式的可视化表达。

- 坐标系配置主要关注坐标转换，起始结束弧度，内外半径等，有关坐标轴刻度，刻度值配置等内容请前往 [图表组件-坐标轴](/options/plots/component/axis)。
...
| **坐标系名称** |                                  **描述**                                  |                                                        **示例**                                                        |
| :------------: | :------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------------------------: |
|   cartesian    |                      笛卡尔坐标系，Ant Design Charts 默认的坐标系。                       |  <img width=100 src="https://mdn.alipayobjects.com/huamei_qa8qxu/afts/img/A*kvZLSLxjnkIAAAAAAAAAAAAADmJ7AQ/original">  |
...
笛卡尔坐标系，Ant Design Charts 中默认的坐标系。
...
const Demo = () => {
  ...
  const config ={
      ...
      positionField: ['Points', 'Rebounds', 'Assists', 'Steals', 'Blocks'],
      colorField: 'name',
      style: {
        lineWidth: 1.5,
        strokeOpacity: 0.4,
...
Theta 坐标系是极坐标系的特化形式，通过固定半径维度、强化角度维度分析能力，专为环形数据可视化设计。在 Ant Design Charts 中主要应用于饼图等角度主导型图表。
