...
与 [threshold](/options/plots/core/scale/threshold) 比例尺类似，quantize 也是将连续数据映射为离散值，但不同的是：

- threshold 比例尺需要手动指定分割点（阈值）
- quantize 比例尺会根据数据域和值域的数量自动计算等宽的区间

与 [quantile](/options/plots/core/scale/quantile) 比例尺的区别在于：
...
以下是一个使用 Ant Design Charts 声明式语法（Ant Design ChartsSpec）配置 quantize 比例尺的完整示例：
...
这个示例展示了如何使用 Ant Design Charts 声明式语法创建一个使用 quantize 比例尺的热力图，包括以下特性：
...
3. **数据域设置**：可以通过设置 `domain` 属性来自定义数据域，例如 `domain: [0, 100]`。如果不设置，Ant Design Charts 会根据数据自动计算合适的数据域。
