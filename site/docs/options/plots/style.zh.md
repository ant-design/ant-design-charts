...
Ant Design Charts 使用 [G](https://g.antv.antgroup.com/) 作为绘图引擎，一些图形的样式配置，如折线图的`line.style`，柱状图的`interval.style`等，还有部分组件的样式配置，如`label.style`， `axis.line${style}`等，都是直接透传 G 的绘图属性。
