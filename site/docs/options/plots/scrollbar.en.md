...
In Ant Design Charts, **Scrollbar** can be used to filter data and can be bound to the x or y channel. Scrollbars are turned off by default. It solves the problem of chart information being too dense to be fully displayed.
...
Scrollbars can be configured at the Mark level. In Ant Design Charts, each Mark has its own scrollbar. If the scales corresponding to marks are synchronized, the scrollbars will also be merged.
