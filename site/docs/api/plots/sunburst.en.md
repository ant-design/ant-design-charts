...
｜ --- ｜ --- ｜ --- ｜
｜`Sunburst.SUNBURST_PATH_FIELD`| Path of current node, up the tree to the least common ancestor, and back down to the given node |*string* |
｜`Sunburst.SUNBURST_ANCESTOR_FIELD`| Ancestor node of current node | *string* |
｜`Sunburst.NODE_ANCESTORS_FIELD`| Ancestor nodes of current node |*object\[]* |
｜`nodeIndex`| Index of nodes at the same level |*number* |
