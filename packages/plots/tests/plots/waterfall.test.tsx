...
describe('Waterfall render', () => {
  ...
  it('classname * loading * style', () => {
    ...
    const testRenderer = create(<Waterfall {...props} />);
  ...
  it('error template', () => {
    ...
    const testRenderer = create(<Waterfall {...props} {...chartProps} />);
    const testInstance = testRenderer.root;
  ...
  it('chart render * chartRef with callback', () => {
    ...
    act(() => {
      render(<Waterfall {...props} {...chartProps} />, container);
    });
    ...
    // 总计会增加一条数据
    expect(chartRef.chart.getData().length).toBe(data.length + 1);
  });
  ...
  it('chartRef with useRef', () => {
    ...
    act(() => {
      render(<Waterfall {...props} {...chartProps} ref={refs} />, container);
    });
    // 总计会增加一条数据
