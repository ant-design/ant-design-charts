...
describe('Bar render', () => {
  ...
  it('error template', () => {
    ...
    const testRenderer = create(<Bar {...props} {...chartProps} />);
    const testInstance = testRenderer.root;
  ...
  it('chart render * chartRef with callback', () => {
    ...
    act(() => {
      ReactDOM.render(<Bar {...props} {...chartProps} />, container);
    });
    expect(chartRef).not.toBeUndefined();
    const canvas = container.querySelector('canvas');
    expect(canvas.width).toBe(200);
    expect(canvas.height).toBe(160);
    expect(chartRef.chart.getData()).toEqual(data.reverse());
  });
  ...
  it('chartRef with createRef', () => {
    ...
    expect(chartRef.current.chart.getData()).toEqual(data.reverse());
  });
  ...
  it('chartRef with useRef', () => {
    ...
    act(() => {
      ReactDOM.render(<Bar {...props} {...chartProps} ref={refs} />, container);
    });
    expect(refs.current.getChart().chart.getData()).toEqual(data.reverse());
  });
});
